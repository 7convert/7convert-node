...
describe('JobsResouce', () => {
    ...
    describe('create()', () => {
        ...
        it('test upload and download files', async () => {
            ...
            let job = await this.sevenConvert.jobs.create({
            ...
            await this.sevenConvert.tasks.upload(uploadTask, stream);

            job = await this.sevenConvert.jobs.wait(job.id);
    ...
    describe('subscribeEvent()', () => {
        ...
        it('test listening for finished event', async () => {
            ...
            const event = await new Promise((resolve) =>  {
                this.sevenConvert.jobs.subscribeEvent(job.id, 'finished', resolve);
            });
        ...
        afterEach(() => {
            if(this.sevenConvert.socket) {
                this.sevenConvert.socket.close();
            }
        });
